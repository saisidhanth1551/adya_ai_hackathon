 ...
 */
server.on(CallToolRequestSchema, async (request) => {
    try {
        ...
        if (error.response?.body?.errors) {
            throw new McpError(ErrorCode.InternalError, `SendGrid API Error: ${error.response.body.errors.map((e) => e.message).join(', ')}`);
        }
        // Handle other errors
